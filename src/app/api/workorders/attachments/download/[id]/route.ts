...
export async function GET(
  ...
  try {
    ...
    console.log('API Route: Starting download for attachment ID:', id);
    ...
    if (contentType === 'application/json' || contentType.includes('json')) {
      console.log('API Route: Detected JSON content type for binary data, correcting to octet-stream');
    ...
    console.error('WorkOrder Attachment Download API Error:', error);
...
function ensureFileExtension(fileName: string, contentType: string, searchParams: URLSearchParams): string {
  ...
  switch (contentType) {
    ...
    case 'audio/wav':
      ...
      break;
    default:
      // For unknown types, try to guess from common patterns
      if (contentType.startsWith('image/')) {
        ...
        // If we can't determine the type, don't add any extension
        // This will preserve the original filename
        console.log('ensureFileExtension: Unknown content type, returning filename as-is');
        return fileName;
      }
      break;
  }
