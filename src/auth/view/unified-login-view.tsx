...
import { signInWithAzureAD } from '../context/azure/action';
...
export function UnifiedLoginView() {
  ...
  const renderSSOButton = () => (
    ...
    >
      {isLoadingSSO ? 'Signing in...' : 'Sign in with Microsoft'}
    </Button>
  );
  ...
  return (
    <>
      ...
      {/* Show both options when 'both' is enabled */}
      {authConfig.isBothEnabled() && (
        <>
          {renderSSOButton()}
          {renderDivider()}
      ...
      {/* Show only SSO when 'sso' is enabled */}
      {authConfig.method === 'sso' && renderSSOButton()}

      {/* Show only local login when 'local' is enabled */}
