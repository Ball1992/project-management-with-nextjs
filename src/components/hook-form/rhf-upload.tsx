...
import { UploadCoverImage } from '../upload/upload-cover-image';
...
export function RHFUploadCoverImage({ name, slotProps, ...other }: RHFUploadProps) {
  const { control, setValue } = useFormContext();
  ...
  return (
    <Controller
      ...
      render={({ field, fieldState: { error } }) => {
        ...
        return (
          <Box {...slotProps?.wrapper}>
            <UploadCoverImage value={field.value} error={!!error} onDrop={onDrop} {...other} />
