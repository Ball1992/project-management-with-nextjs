...
export function AboutView() {
  ...
  // Helper function to get current translation (not a useCallback to avoid dependency issues)
  const getCurrentTranslation = () => {
    if (!content) return null;
    
    // Check if translations exists and is an array
  ...
  const fetchLanguages = useCallback(async () => {
    try {
      ...
      if (languagesData && Array.isArray(languagesData)) {
        ...
        // Set default language to first active language or 'en'
  ...
  const fetchContent = useCallback(async () => {
    try {
      setLoading(true);
      const data = await contentService.getContentBySlug('about-us', currentLang);
  ...
  const handleLanguageChange = useCallback((event: React.MouseEvent<HTMLElement>, newLang: string | null) => {
    if (newLang && newLang !== currentLang) {
      ...
      setEditing(false); // Exit edit mode when switching language
    }
  }, [currentLang]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Use translation API to update content for specific language
  ...
  useEffect(() => {
    if (content) {
      // Get translation data directly without using callback
      const translations = Array.isArray(content.translations) ? content.translations : [];
      const translation = translations.find(t => t.languageCode === currentLang);
