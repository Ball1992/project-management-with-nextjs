...
export function UserFilters({ open, canReset, onOpen, onClose, filters, options }: Props) {
  ...
  const handleFilterStatusTypes = useCallback(
    (newValue: string) => {
      const checked = currentFilters.statusTypes?.includes(newValue)
        ? currentFilters.statusTypes?.filter((value) => value !== newValue)
        : [...(currentFilters.statusTypes || []), newValue];

      updateFilters({ statusTypes: checked });
    },
    [updateFilters, currentFilters.statusTypes]
  );
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {options.statusTypes.map((option) => (
        <FormControlLabel
          ...
          control={
            <Checkbox
              //checked={currentFilters.statusTypes.includes(option)}
              onClick={() => handleFilterStatusTypes(option)}
            />
          }
          label={option}
        />
