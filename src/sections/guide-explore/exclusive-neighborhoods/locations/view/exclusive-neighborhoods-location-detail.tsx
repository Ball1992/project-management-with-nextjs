...
export const ExclusiveNeighborhoodsLocationSchema = zod.object({
...
export type ExclusiveNeighborhoodsLocationSchemaType = zod.infer<typeof ExclusiveNeighborhoodsLocationSchema>;

// ----------------------------------------------------------------------

type Props = {
  currentLocation?: IExclusiveNeighborhoodsLocation;
};

export function ExclusiveNeighborhoodsLocationDetailView({ currentLocation }: Props) {
  ...
  const [currentLocationData, setCurrentLocationData] = useState<IExclusiveNeighborhoodsLocation | null>(currentLocation || null);
  ...
  const defaultValues: ExclusiveNeighborhoodsLocationSchemaType = {
    code: currentLocation?.code || '',
    sortOrder: currentLocation?.sortOrder || 0,
    isActive: currentLocation?.isActive ?? true,
    // Translations
    name: getCurrentTranslation()?.name || currentLocation?.name || '',
    description: getCurrentTranslation()?.description || currentLocation?.description || '',
  };

  const methods = useForm<ExclusiveNeighborhoodsLocationSchemaType>({
    defaultValues,
  });
  ...
  useEffect(() => {
    if (isEdit && currentLocation && selectedLanguage) {
      const fetchLocationDetail = async () => {
        try {
          setLoading(true);
          const detailedLocation = await exclusiveNeighborhoodsLocationsService.getLocation(currentLocation.id, selectedLanguage.code);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (isEdit && currentLocation) {
        // For update
        const updateData: UpdateExclusiveNeighborhoodsLocationDto = {
          ...baseData,
          translations: translationData,
        };
        
        await exclusiveNeighborhoodsLocationsService.updateLocation(currentLocation.id, updateData);
        toast.success('Location updated successfully!');
        
        // Reload data instead of redirecting
        if (selectedLanguage) {
          const updatedLocation = await exclusiveNeighborhoodsLocationsService.getLocation(currentLocation.id, selectedLanguage.code);
          setCurrentLocationData(updatedLocation);
        }
      } else {
        // For create
        const createData: CreateExclusiveNeighborhoodsLocationDto = {
          ...baseData,
          name: data.name,
          translations: translationData,
        };
        
        const response = await exclusiveNeighborhoodsLocationsService.createLocation(createData);
        toast.success('Location created successfully!');
        
        // Redirect to edit page with the new ID
        router.push(`/menu/guide-explore/exclusive-neighborhoods/locations/${response.id}`);
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await exclusiveNeighborhoodsLocationsService.deleteLocation(currentLocation.id);
      toast.success('Location deleted successfully!');
      router.push('/menu/guide-explore/exclusive-neighborhoods/locations');
    } catch (err) {
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        ...
        backHref={'/menu/guide-explore/exclusive-neighborhoods/locations'}
        links={[
          { name: 'Dashboard', href: '/dashboard' },
          { name: 'Guide Explore' },
          { name: 'Exclusive Neighborhoods' },
          { name: 'Locations', href: '/menu/guide-explore/exclusive-neighborhoods/locations' },
